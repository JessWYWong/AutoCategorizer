...
void XMLCategorizer::loadFromXMLRecursive(TXMLEngine* xml, XMLNodePointer_t xnode, CategoryNode* cnode)
{
   // Get the split information from xml.
    ...
    else
    {
        ...
        if(cnode->mother==0)
        {
            cleft->key = Form("%s < %7.3f", splitVarName.Data(), splitVal);
            cright->key = Form("%s > %7.3f", splitVarName.Data(), splitVal);
        }
        else
        {
            cleft->key = cnode->key+Form(" && %s < %7.3f", splitVarName.Data(), splitVal);
            cright->key = cnode->key+Form(" && %s > %7.3f", splitVarName.Data(), splitVal);
        }
